...
class Rwkv7TimeMix(nnx.Module):
  ...
  def __call__(
    ...
    # The reciprocal norm is computed in float32 whatever the projection dtype:
    # in float16 the 1e-24 clamp would itself flush to zero, so an all-zero
    # vector would come back NaN rather than zero.
    kk_f32 = jnp.asarray(kk, jnp.float32)
    inv_norm = jax.lax.rsqrt(jnp.maximum(jnp.sum(jnp.square(kk_f32), axis=-1, keepdims=True), 1e-24))
    kk = jnp.asarray(kk_f32 * inv_norm, kk.dtype)
    k = k * (1 + (a - 1) * self.k_a[...])
    ...
    if self.recurrent_state is not None:
      ...
      # A call narrower than the cache (e.g. batch-1 prefill) writes back only the rows it owns.
      shift_out = jnp.asarray(_last_valid(x, shift_in, valid_lens), self.conv_state[...].dtype)
      self.recurrent_state[...] = self.recurrent_state[...].at[:batch].set(new_state)
      self.conv_state[...] = self.conv_state[...].at[:batch].set(shift_out)
